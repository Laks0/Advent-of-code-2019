...
def setPositions(path):
	...
	for m in path:
		...
		if movement[0] == "U":
			for i in range(int(movement[1])):
				lastPos = positions[len(positions) - 1]
				positions.append([lastPos[0], lastPos[1]-1])
		if movement[0] == "D":
			for i in range(int(movement[1])):
				lastPos = positions[len(positions) - 1]
				positions.append([lastPos[0], lastPos[1]+1])
...
positions1 = setPositions(path1)
positions2 = setPositions(path2)
...
b = set(hash1.keys()).intersection(set(hash2.keys()))

distance = None

for i in b:
	d = hash1[i] + hash2[i]
	if distance == None or distance >= d and d != 0:
		distance = d
		print(d)